...
**Por qué a veces no se descontó stock:** (1) El webhook no estaba configurado en la app de ML (URL en DevCenter). (2) La URL del webhook no es accesible desde internet (localhost, firewall). (3) La variante no estaba vinculada a un producto en el momento de la venta. (4) Error puntual (timeout, 500) y ML dejó de reintentar.

**Cómo sincronizar ahora:** En **Integraciones → Mercado Libre** (con la cuenta conectada), usá el botón **"Sincronizar ventas ML (descontar stock)"**. Eso llama a `POST /api/mercadolibre/sync-orders`, obtiene las últimas órdenes de ML y por cada ítem ejecuta la misma lógica que el webhook (`processMercadoLibreSale`). Las órdenes ya procesadas no se descuentan dos veces (idempotencia por `reference_id`).
...
**Venta fiada (pago 0):** Si en el POS registrás una venta con monto pagado 0 (todo fiado), se registra igual: un solo movimiento de stock (descuento) y una deuda en la cuenta del cliente. Para canal PHYSICAL también se envía el stock actualizado a ML; no se descuenta dos veces.
...
| `/api/mercadolibre/sync-orders` | POST | Sincroniza ventas ML: obtiene órdenes recientes y descuenta stock para las que no se había procesado el webhook. Idempotente. |
